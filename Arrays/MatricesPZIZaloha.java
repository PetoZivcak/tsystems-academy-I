...
public class MatricesPZIZaloha {
    ...
    private static int[][] multiply(int[][] matrixA, int[][] matrixB) {
        ...
        if (multCheck == true) {
            ...
            int i=0;
            int c=0;
            int[][] myTempRes = new int[matrixB[0].length][transposedMatrixA.length];
            while (a < matrixB[0].length) {

                for (i = 0; i < transposedMatrixA.length; i++) {

                    myTempRes[c][a] = myTempRes[c][a] + transposedMatrixA[i][c] * matrixB[i][a];
                }
                a = a + 1;
